var Jobie = function(){
	...
	var handleShowPass1 = function(){
		jQuery('.show-pass').on('click',function(){
			jQuery(this).toggleClass('active');
			if(jQuery('#dz-password2').attr('type') == 'password'){
				jQuery('#dz-password2').attr('type','text');
			}else if(jQuery('#dz-password2').attr('type') == 'text'){
				jQuery('#dz-password2').attr('type','password');
			}
		});
	}
...
function getLocation()
{
	if (navigator.geolocation) {
		navigator.geolocation.getCurrentPosition(function(position) {
		  ...
		  getPage('main','get_loc/' + latitude + '/' + longitude,'');
		}, function(error) {
		  // Error callback
		  console.error(error);
		});
