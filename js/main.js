...
function getPage(controller,func,loc_id)
{
		...
		xmlhttp.open("GET",'/'+controller+'/'+func,true); 
		xmlhttp.send();
}

function postREST(form,ids)
{
	if (window.XMLHttpRequest)
		{// code for New Browsers
		xmlhttp=new XMLHttpRequest();
	...
	xmlhttp.open(formData.method,formData.action,true);
	xmlhttp.setRequestHeader("Content-type", "application/json");
	xmlhttp.send(new FormData(formData));
}




function postForm(form,ids)
{
	if (window.XMLHttpRequest)
		{// code for New Browsers
		xmlhttp=new XMLHttpRequest();
...
function postForm1(form,displayIds,action,method)
{
	if (window.XMLHttpRequest)
		{// code for New Browsers
		xmlhttp=new XMLHttpRequest();
	...
	xmlhttp.onreadystatechange=function()
		...
		document.getElementById(displayIds).innerHTML=xmlhttp.responseText;
		}
		};
	var formData = document.getElementById(form);
...
function sign(onoff)
{
	if (onoff == true)
	{
		document.getElementById("ov1").className = "sign-on";
		document.getElementById("ov2").className = "sign-on"; 
		topFunction();
	} else
	{
		document.getElementById("ov1").className = "sign-off";
		document.getElementById("ov2").className = "sign-off"; 
	}
}	
